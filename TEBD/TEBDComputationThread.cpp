...
void TEBDComputationThread::Calculate()
{
	...
	else
	{
		...
		m_iTEBDobj.CalculateImaginaryTimeEvolution(runSuccess, H, 4000);
	}
	...
	if (runSuccess)
	{
		...
		runSuccess = m_iTEBDobj.CalculateRealTimeEvolution(Ht, static_cast<int>(ceill(2000 * m_Options.maxTime)), 0.001);
	}
